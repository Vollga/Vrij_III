...
public class ReplaceSelected : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        using (new EditorGUI.DisabledGroupScope(sourceObject == null))
        {
            ...
            if (GUILayout.Button(new GUIContent(" Replace " + Selection.gameObjects.Length + " GameObject" + (Selection.gameObjects.Length > 1 ? "s" : ""), EditorGUIUtility.IconContent("Refresh").image)))
            {
    ...
    {
        Object sourcePrefab = PrefabUtility.GetCorrespondingObjectFromOriginalSource(source);

        if (sourcePrefab)
        {
            isPrefab = true;
            if (PrefabUtility.GetPrefabAssetType(source) == PrefabAssetType.Model) isPrefab = false;
        }
